...
def generate_formatted_data(parsed_text: str) -> ParsedReceipt:
    """
    ...
    try:
            ...
            messages=[
                {
                    ...
                    - If you cannot extract required fields (merchant_name, total_amount), set confidence_score < 0.5
                    """
                },
                {
                    "role": "user",
        ...
        try:
            receipt = ParsedReceipt(**parsed_json)
            return receipt
        except ValidationError as e:
            raise AIExtractionError(f"output doesn't match schema: {e}")
